...
function issueKey(j: Job): string {
  const prefix = KEY[j.workload] ?? "JOB";
  const n = j.issue_number ?? (Number(j.job_id.replace(/\D/g, "").slice(-3)) || 0);
  return `${prefix}-${n}`;
}

// Adapted from the Linear clone's IssueRow: dense h-10 rows,
// priority icon → mono key → status icon → title → meta.
      ...
      {jobs.map((j) => {
        ...
        return (
          <button
            key={j.job_id}
            onClick={() => onSelect(j)}
            className="group flex h-10 w-full items-center gap-3 border-b border-border px-4 text-left text-sm transition-colors last:border-0 hover:bg-muted/50"
          >
